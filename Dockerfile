...
RUN apt update && \
apt install -y nginx sudo samba jq openssh-server zsh curl wget cron git && \
...
COPY root_path/ /
RUN chmod a+x /shell -R && chmod a+x /gdrive/* && mkdir -p /log /home/root && chmod 7777 /nginx/ -R
# Configs
WORKDIR /home/root
